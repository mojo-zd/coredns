...
func newPrefetchResponseWriter(server string, state request.Request, c *Cache) *ResponseWriter {
	...
	// The protocol of the client triggering a cache prefetch doesn't matter in
	// a prefetch request. The address type is used by request.Proto to
	// determine the response size, and using TCP ensures the message isn't
